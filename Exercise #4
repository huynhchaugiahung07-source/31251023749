...
public class BankAccount
{
    ...
    public bool Withdraw(decimal amount, string inputPin)
    {
        ...
        {
            _failedAttempts++;
            Console.WriteLine("Error: Invalid PIN code: " + _failedAttempts);
        ...
        {
            _failedAttempts = 0; // Reset failed attempts on successful PIN entry
            if (amount <= 0)
            {
            ...
            _balance -= amount;
            Console.WriteLine($"Withdrawal of {amount:C} successful. New balance: {_balance:C}");
            return true;
        }
    }
    ...
    {   
        ...
        // Check if new PIN is valid (not null, 4 chars long, all digits)
...
class Program
{
    ...
    {
        // Thiết lập chuẩn tiền tệ sang USD ($)
